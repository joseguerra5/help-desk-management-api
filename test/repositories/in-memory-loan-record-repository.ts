...
export class InMemoryLoanRecordRepository implements LoanRecordRepository {
  ...
  async count({ from, status }: Count): Promise<number> {
    const amount = this.items.filter((item) => {
      ...
      const matchesDate = from ? new Date(item.ocurredAt) >= new Date(from) : true
  ...
  async findManyByCooperatorId(cooperatorId: string, { page, status }: PaginationLoanRecordParams): Promise<LoanRecord[]> {
    ...
    if (cooperatorId) {
      filteredItems = filteredItems.filter(item => item.cooperatorId.toString() === cooperatorId);
    }
    ...
    filteredItems = filteredItems.sort((a, b) => b.ocurredAt.getTime() - a.ocurredAt.getTime());
