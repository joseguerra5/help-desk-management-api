...
export class PrismaCooperatorRepository implements CooperatorRepository {
  ...
  async findMany({ page, search, status }: PaginationCooperatorParams): Promise<Cooperator[]> {
    const cooperators = await this.prisma.cooperator.findMany({
      where: {
        departureDate: status === "active" ? null : status === "inactive" ? { not: null } : undefined,
        OR: search ? [{ employeeId: { contains: search, mode: "insensitive" } }, { userName: { contains: search, mode: "insensitive" } }] : undefined
      },
      orderBy: {
        createdAt: "desc"
      },
  ...
  async save(cooperator: Cooperator): Promise<void> {
    ...
    await this.prisma.cooperator.update({
      where: {
        id: data.id
      },
      data,
