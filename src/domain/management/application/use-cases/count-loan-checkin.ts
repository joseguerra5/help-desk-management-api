...
type CountLoanCheckinUseCaseReponse = Either<CredentialDoNotMatchError, {
  amount: number
}>
...
export class CountLoanCheckInUseCase {
  ...
  async execute(): Promise<CountLoanCheckinUseCaseReponse> {
    ...
    const amount = await this.loanRecordRepository.count({ from: new Date(last30Days), status: "CHECK_IN" })
    return right({
      amount
    })
  }
